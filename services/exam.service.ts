...
export class ExamService {
  async createExam(userId: string, title: string, totalQuestions: number, subjects: string[], options?: { description?: string; duration?: number; passPercentage?: number; difficulty?: ('easy' | 'medium' | 'hard')[] }) {
    ...
    const candidates = await db.question.findMany({
      where: { topic: { subject: { id: { in: subjects } } }, ...(options?.difficulty?.length ? { difficulty: { in: options.difficulty } } : {}) },
      include: { alternatives: true },
    });
    const selected = [...candidates].sort(() => Math.random() - 0.5).slice(0, Math.min(totalQuestions, candidates.length));
    ...
    return db.exam.create({
      ...
      include: { subjects: true, questions: { include: { question: { include: { alternatives: true } } }, orderBy: { order: 'asc' } } },
    });
  }
  ...
  async getExamById(id: string, userId?: string) {
    const exam = await db.exam.findUnique({
      where: { id },
      include: { questions: { include: { question: { include: { alternatives: true } } }, orderBy: { order: 'asc' } }, results: true },
    });
    if (!exam) return null;
  ...
  async submitExam(examId: string, userId: string, answers: Array<{ questionId: string; selectedAnswer: string }>) {
    ...
    const uniqueAnswers = new Map(answers.filter((answer) => validQuestionIds.has(answer.questionId)).map((answer) => [answer.questionId, answer]));
    let correctCount = 0;
    ...
    return db.examResult.create({ data: { examId, totalQuestions: exam.totalQuestions, correctAnswers: correctCount, wrongAnswers: wrongCount, percentage } });
  }

  async getUserExams(userId: string) {
    return db.exam.findMany({ where: { userId }, include: { results: true }, orderBy: { createdAt: 'desc' } });
  }
}
